...
export function CartProvider({ children }) {
  ...
  const buyNow = async (productId, variantId, quantity = 1) => {
    ...
    try {
      ...
      // Get AnyTrack click_id to pass to Wix checkout
      let clickId = null;
      if (typeof window !== "undefined" && window.AnyTrack) {
      ...
      // Append click_id to checkout URL
      let checkoutUrl = redirectSession.fullUrl;
      if (clickId) {
        checkoutUrl += `${checkoutUrl.includes("?") ? "&" : "?"}atclid=${clickId}`;
      }

      console.log("Step 3 - Redirecting with clickId:", checkoutUrl);
