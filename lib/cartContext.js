...
export function CartProvider({ children }) {
  ...
  const buyNow = async (productId, variantId, quantity = 1) => {
    ...
    try {
      ...
      // Save atclid + checkoutId to Supabase as backup (fire and forget)
      if (clickId && checkoutId) {
        fetch("/api/save-atclid", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ checkoutId, atclid: clickId }),
        }).catch(() => {});
        console.log("Supabase save fired — checkoutId:", checkoutId, "atclid:", clickId);
      }
