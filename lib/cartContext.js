...
// Single client instance persisted in module scope
let clientInstance = null;

function getClient() {
  if (!clientInstance) {
    const tokens = JSON.parse(Cookies.get("wix_session") || "null");
    ...
    clientInstance.auth.onTokensChanged((tokens) => {
      Cookies.set("wix_session", JSON.stringify(tokens), { expires: 2 });
    });
  }
